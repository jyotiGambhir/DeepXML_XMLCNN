...
class FastAttentionXML(object):
    ...
    def __init__(self, labels_num, data_cnf, model_cnf, tree_id=''):
        ...
        self.model_path = os.path.join(model_cnf['path'], '{}-{}{}'.format(model_name,data_name,tree_id))
        ...
        self.groups_path = os.path.join(model_cnf['path'], '{}-{}{}-cluster'.format(model_name,data_name,tree_id))
  ...
  #      logger.info("inside train level {}".format(level))
        model_cnf, data_cnf = self.model_cnf, self.data_cnf
        if level == 0:
            while not os.path.exists('{}-Level-{}.npy'.format(self.groups_path,level)):
 #               logger.info("level == 0 {} {}".format(self.groups_path, level))
                time.sleep(30)
            groups = np.load('{}-Level-{}.npy'.format(self.groups_path,level))
#            logger.info("group type {}".format(type(groups)))
   ...
   #         logger.info("train loader done {}".format(type(train_loader)))
            valid_loader = DataLoader(MultiLabelDataset(valid_x, valid_y, training=False),
                                      model_cnf['valid']['batch_size'], num_workers=4)
    #        logger.info("valid loader done {}".format(type(valid_loader)))
            model = Model(AttentionRNN, labels_num=labels_num, model_path='{}-Level-{}'.format(self.model_path,level),
                          emb_init=self.emb_init, **data_cnf['model'], **model_cnf['model'])
     #       logger.info("model created , model type :{}".format(type(model)))
            if not os.path.exists(model.model_path):
      #          logger.info("model path {}".format(model.model.path))
       #         logger.info('Training Level-{}, Number of Labels: {}',format(level,labels_num))
                model.train(train_loader, valid_loader, **model_cnf['train'][level])
                model.optimizer = None
                logger.info('Finish Training Level-{}'.format(level))
            self.models[level] = model
            logger.info('Generating Candidates for Level-{}, Number of Labels: {}, Top: {}'.format(level+1,labels_num,self.top))
            ...
            if level < self.level - 1:
                while not os.path.exists('{}-Level-{}.npy'.format(self.groups_path,level)):
                    logger.info("{} {}".format(self.groups_path, level))
                    time.sleep(30)
                groups = np.load('{}-Level-{}.npy'.format(self.groups_path,level))
                train_y, valid_y = self.get_mapping_y(groups, self.labels_num, train_y, valid_y)
                ...
                last_groups = np.load('{}-Level-{}.npy'.format(self.groups_path,level-1))
            ...
            model = XMLModel(network=FastAttentionRNN, labels_num=labels_num, emb_init=self.emb_init,
                             model_path='{}-Level-{}'.format(self.model_path,level), **data_cnf['model'], **model_cnf['model'])
            if not os.path.exists(model.model_path):
                logger.info('Loading parameters of Level-{} from Level-{}'.format(level,level-1))
                ...
                logger.info('Training Level-{}, Number of Labels: {}, Candidates Number: {}'.format(level,labels_num,train_loader.dataset.candidates_num))
                model.train(train_loader, valid_loader, **model_cnf['train'][level])
    ...
    def predict_level(self, level, test_x, k, labels_num):
        ...
        if level == 0:
            logger.info('Predicting Level-{}, Top: {}'.format(level,k))
            if model is None:
                model = Model(AttentionRNN, labels_num=labels_num, model_path='{}-Level-{}'.format(self.model_path,level),
                              emb_init=self.emb_init, **data_cnf['model'], **model_cnf['model'])
            ...
            if level == self.level - 1:
                groups = np.load('{}-Level-{}.npy'.format(self.groups_path,level-1))
            else:
            ...
            if model is None:
                model = XMLModel(network=FastAttentionRNN, labels_num=labels_num,
                                 model_path='{}-Level-{}'.format(self.model_path,level),
                                 emb_init=self.emb_init, **data_cnf['model'], **model_cnf['model'])
