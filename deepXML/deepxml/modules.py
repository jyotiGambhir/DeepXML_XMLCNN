...
__all__ = ['Embedding', 'LSTMEncoder', 'MLAttention',
           'AttentionWeights', 'FastMLAttention', 'MLLinear']
...
class LSTMEncoder(nn.Module):
    ...
    def forward(self, inputs, lengths, **kwargs):
        ...
        cell_init, hidden_init = init_state[:init_state.size(
            0) // 2], init_state[init_state.size(0) // 2:]
...
class MLAttention(nn.Module):
    ...
    def forward(self, inputs, masks):
        ...
        attention = self.attention(inputs).transpose(
            1, 2).masked_fill(~masks, -np.inf)  # N, labels_num, L
        ...
        if os.path.isfile(filepath):
            ...
            attn = attn_prev + attn_new
        else:
            attn = np.sum(attention.cpu().detach().numpy(), axis=0)
...
class AttentionWeights(nn.Module):
    ...
    def __init__(self, labels_num, hidden_size, device_ids=None):
        ...
        group_size, plus_num = labels_num // len(
            device_ids), labels_num % len(device_ids)
        self.group = [group_size + 1] * plus_num + \
            [group_size] * (len(device_ids) - plus_num)
        ...
        self.group_offset, self.hidden_size = np.cumsum(
            [0] + self.group), hidden_size

    def forward(self, inputs: torch.Tensor):
        outputs = torch.zeros(
            *inputs.size(), self.hidden_size, device=inputs.device)
...
class FastMLAttention(nn.Module):
    ...
    def __init__(self, labels_num, hidden_size, parallel_attn=False):
        ...
        if parallel_attn:
            self.attention = nn.Embedding(
                labels_num + 1, hidden_size, sparse=True)
            nn.init.xavier_uniform_(self.attention.weight)

    def forward(self, inputs, masks, candidates, attn_weights: nn.Module):
        masks = torch.unsqueeze(masks, 1)   # N, 1, L
        attn_inputs = inputs.transpose(1, 2)    # N, hidden, L
        attn_weights = self.attention(candidates) if hasattr(
            self, 'attention') else attn_weights(candidates)
        attention = (attn_weights @ attn_inputs).masked_fill(1.0 -
                                                             masks, -np.inf)  # N, sampled_size, L
