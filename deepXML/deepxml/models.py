...
class XMLModel(Model):
    ...
    def __init__(
            ...
            hidden_size,
            device_ids=None,
            attn_device_ids=None,
            most_labels_parallel_attn=80000,
            **kwargs):
        ...
        if not parallel_attn:
            self.attn_weights = AttentionWeights(
                labels_num, hidden_size * 2, attn_device_ids)
        self.model = nn.ModuleDict(
            {'Network': self.network.module, 'AttentionWeights': self.attn_weights})
        self.state['best'] = {}

    def train_step(self,
                   train_x: Tuple[torch.Tensor,
                                  torch.Tensor],
                   train_y: torch.Tensor):
        self.optimizer.zero_grad()
        train_x, candidates = train_x
        self.model.train()
        scores = self.network(train_x, candidates=candidates,
    ...
    def predict_step(self,
        ...
        self.model.eval()
        with torch.no_grad():
            scores = torch.sigmoid(self.network(
                data_x, candidates=candidates, attn_weights=self.attn_weights))
            scores, labels = torch.topk(scores * group_scores.cuda(), k)
            return scores.cpu(), candidates[np.arange(
                len(data_x)).reshape(-1, 1), labels.cpu()]
