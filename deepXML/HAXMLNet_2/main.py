...
def main(data_cnf, model_cnf, mode, tree_id):
    tree_id = '-Tree-{tree_id}'.format(tree_id=tree_id) if tree_id is not None else ''

    yaml = YAML(typ='safe')
    data_cnf, model_cnf = yaml.load(Path(data_cnf)), yaml.load(Path(model_cnf))
    model, model_name, data_name = None, model_cnf['name'], data_cnf['name']
    model_path = os.path.join(model_cnf['path'], '{}-{}{}'.format(model_name,data_name,tree_id))
    ...
    if mode is None or mode == 'eval':
        ...
        output_res(data_cnf['output']['res'], '{}-{}{}'.format(model_name,data_name,tree_id), scores, labels)
