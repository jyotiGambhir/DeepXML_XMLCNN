...
def load_word2vec(params):
    ...
    if params.model_type == 'GoogleNews':
        ...
        model_name = join(model_dir, 'glove.840B.%dd.txt' % (params.num_features))
        print(model_name)
        assert(exists(model_name))
        ...
        for line in open(model_name, 'r'):
            ...
            #	if i == '.':
            #		k+=1
            word, vec = tmp[0],  np.array([ float(i) for i in tmp[1:]]).astype(np.float)
            #print(len(vec))
            if len(vec) != params.num_features:
            	print(vec, len(vec))
            	print(word, tmp[:], len(tmp[1:]))
